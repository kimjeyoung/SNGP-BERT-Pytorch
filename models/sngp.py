...
class SNGP(nn.Module):
    ...
    def gp_layer(self, gp_inputs, last_epoch=True):
    ...
    def forward(self, input_ids, token_type_ids: Optional[Tensor] = None,
                ...
                epoch: int = 0):
        last_epoch = True if epoch == self.final_epochs else False
        ...
        gp_feature, gp_output = self.gp_layer(cls_output, last_epoch=last_epoch)
        if return_gp_cov:
